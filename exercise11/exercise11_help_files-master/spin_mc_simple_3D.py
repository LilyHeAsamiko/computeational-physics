...
def main():
    ...
    mapping = zeros((grid_side,grid_side,grid_side),dtype=int) # mapping
    ...
    for i in range(grid_side):
        for j in range(grid_side):
            for k in range(grid_side):
                    mapping[i, j, k]=ii
                    inv_map.append([i,j,k])
                    ii += 1
 

    # get the 6 nearest neighbors of the walkers which are at the grids: 
    #[9,1,90,10],[0,2,90,10]...[19,11,0,2],[10,12,1,3]... avoiding boundary problem
    for i in range(grid_side):
        for j in range(grid_side):
            for k in range(grid_side):
                k1=mapping[i, j,(k-1) % grid_side]
                k2=mapping[i, j,(k+1) % grid_side]
                j1=mapping[i, j-1,k]
                j2=mapping[i, j+1,k]
                i1=mapping[(i-1) % grid_side,j, k]
                i2=mapping[(i+1) % grid_side,j, k]
                Walkers.append(Walker(spin=0.5,
                                  nn=[i1,i2,j1,j2,k1,k2],
                                  dim=dim,
                                  coords = [i, j, k]))
