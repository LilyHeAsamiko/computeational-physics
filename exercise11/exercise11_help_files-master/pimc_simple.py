...
#            V += De*(1-exp(-a*(r-r_e)))**2-De
#
#    Vext = external_potential(Walker)
    
#    return V+Vext

def potential(Walker):#Lennard-Jones(Walker):
    V = 0.0
    kb = 3.16681*10**(-6)
    p = 8.6*kb
    sigma = 0.281
    ...
    for i in range(Walker.Ne-1):
        for j in range(i+1,Walker.Ne): 
            r = sqrt(sum((Walker.Re[i]-Walker.Re[j])**2))
#    r = sqrt(sum((Walker.Re[0]-Walker.Re[1])**2))
    V += 4*p*[(sigma/array(r))**12-(sigma/array(r))**6]
...
#    title ('cumulant approximation')
    title ('quantum dot')
    savefig('quantum dot.pdf', dpi = 200)
    Eb = Eb[20:]
