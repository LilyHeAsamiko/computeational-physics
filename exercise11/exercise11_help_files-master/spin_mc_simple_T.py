...
def ising(Nblocks,Niters,Walkers,beta,Tamperature):
    M = len(Walkers)
    Eb = zeros((Nblocks,))
    Ec = zeros((Nblocks,))
    Magb = zeros((Nblocks,))
...
def main():
    ...
    T = linspace(0.5,6,Nblocks) #tamperature
    beta = 1.0/T
    """
    Notice: Energy is measured in units of k_B, which is why
            beta = 1/T instead of 1/(k_B T)
    """
    Walkers, Eb, Acc, Magb, C, chi= ising(Nblocks,Niters,Walkers,beta, T)

    figure()
    plot(Eb)
    title('With T changing Metropolis Monte Carlo Ising model')
    savefig('With T changing Metropolis Monte Carlo Ising model', dpi = 200)
    Eb = Eb[eq:]
    ...
    figure()
    plot(Magb)
    title('With T changing Metropolis Monte Carlo Ising model Magnetizaton')
    savefig('With T changing Metropolis Monte Carlo Ising model Magnetizaton', dpi = 200)
    Magb = Magb[eq:]
    ...
    figure()
    plot(C)
    title('With T changing Metropolis Monte Carlo Ising model Capacity')
    savefig('With T changing Metropolis Monte Carlo Ising model Capacity', dpi = 200)
    C = C[eq:]
    ...
    figure()
    plot(chi)
    title('With T changing Metropolis Monte Carlo Ising model Susceptibility')
    savefig('With T changing Metropolis Monte Carlo Ising model Susceptibility', dpi = 200)
    chi = chi[eq:]
    ...
    f = open('observable quantities_T.txt','w+')
