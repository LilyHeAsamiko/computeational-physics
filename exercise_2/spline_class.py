...
class spline(object):
    ...
    def test2d(self, x, y, z, tol):
        ...
        plt.savefig('spline 2D error.pdf',dpi=200)
        ax22.pcolor(x, y, z)
        ...
        # round and find the closest integer for i
            ...
            for valy in y:
                ...
                for valz in z:
                    ...
                    # calculate A and B according to Hermite cubic splines 2D
    ...
    def test3d(self, X, Y, Z, tol):
        ...
        # the colored norm2 of the error        
        plt.title('colored error')
        plt.savefig('spline_colored_3D error.pdf',dpi=200)
...
def main():
    ...
    tol = 0.5
    err1 = spl1d.test1d(xx, spl1d.eval1d(xx)-np.sin(xx), tol)
    ...
    plt.savefig('spline_2D_11_11.pdf',dpi=200)
    # colored with z_value
    ax2d3.pcolor(X,Y,Z1)
    plt.title('color of interpolation 2D with 11*11 grids')
    plt.savefig('spline_colored_2D_11_11.pdf',dpi=200)    
    #ax2d3.contourf(X,Y,Z)
    
    spl2d=spline(x=x,y=y,f=Z1,dims=2)
    ...
    plt.savefig('spline_2D_51_51.pdf',dpi=200)
    # colored with z_value
    ax2d4.pcolor(X,Y,Z2)
    plt.title('color of interpolation 2D with 51*51 grids')
    plt.savefig('spline_colored_2D_51_51.pdf',dpi=200)
    tol = 1    
    err2 = spl2d.test2d(X, Y, Z2-fun2(X,Y), tol)
    ...
    X,Y= np.meshgrid(x,y)
    fig3d=plt.figure()
    ax3d=fig3d.add_subplot(121)
    ax3d.pcolor(X,Y,F1[...,int(len(z)/2)])
    spl3d=spline(x=x,y=y,z=z,f=F1,dims=3)
